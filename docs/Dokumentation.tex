...
\begin{itemize}
    ...
    \item \textbf{Authentifizierung: Keycloak} dient als Identity- und Access-Management-Lösung. Die Wahl fiel darauf, da es standardisierte Authentifizierungsverfahren  abbildet, rollenbasierte Zugriffe verwalten kann und ohne Eigenimplementierung eine robuste, sicherheitsfokussierte Benutzerverwaltung ermöglicht. Zusätzlich ist Keycloak opensource und bietet die Möglichkeit sämtliche Nutzerdaten zu verwaltung und auch dementsprechend im System zu nutzen. Was eine komplexe Implementierung dieser Funktionen umgeht und e
...
Eine große Hilfe beim Entwickeln einer Softwareanwendung ist eine Strukturelle Übersicht über das Programm. Eine Umsetzungsmöglichkeit bietet hierfür ein UML Klassendiagramm. \\

Das in Abb. \ref{fig:classdiagramm} gezeigte Klassendiagram bietet einen Überblick über den strukturellen Aufbau der Applikation. Zu sehen sind die Klassen User und die davon abgeleitete Klasse Admin, deren Instanzen von der Klasse Authentication initialisiert und mit Nutzerdaten gefüttert werden. \\
...
Um sich das Verhalten der Anwendung genauer vorstellen zu können bieten sich Verhaltensdiagramme an, welche besondere Abläufe beschreiben. Hierzu wurde ein Sequenzdiagramm erstellt, welches exemplarisch den Ablauf des sendens eines Layouts darstellt. 
...
Da dieses Diagramm bereits alle Akteure und Komponenten des Systems (bis auf Keycloak) darstellt und alle weiteren möglichen Verhaltensmuster diesem weitgehend gleichen genügt diese Anscht um ein Bild vom Verhalten der Anwendung zu erlangen. 
...
Aus diesem Modell gehen die für die Speicherung der Daten notwendigen Tables und deren Atrribute hervor. Alle Nutzerdaten werden im Table User gespeichert (Admins gelten als Nutzer mit Is Admin = True), Layouts mit Metadaten im Table Layout, und deren Elemente mit Position und Größe im Table Elemente. Wichtig ist hierbei das Feld URI im Table Elemente, welches gesetzt wird wenn das Element vom Typ Bild oder QR Code ist. Im Falle eines Bildes verweist es auf ein im Anwendungsordner gespeichertes Bild, im Falle eines QR Codes ist die URI die des Codes selbst. In allen weiteren Fällen gibt das Attribut Typ den genauen Typ an den das Element hat, z.B. Nachname oder Firma.
...
\section{Interaktive API-Dokumentation (Swagger UI)}
...
\subsection{Anleitung zu Benutzung}
...
Unsere API verwendet Keycloak zur Absicherung bestimmter Endpunkte. Um diese testen zu können, muss man sich autehntifizieren:
